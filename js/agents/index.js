 ...
 *       交叉验证信息，去重整理后输出结构化报告
 */
function createResearchAgent(tools, llm) {
 ...
 *       从多个维度对比，给出择业建议
 */
function createCompareAgent(tools, llm) {
  return new ReActAgent({
    name: '对比Agent',
    role: 'Offer 对比分析师',
 ...
 *       制定针对性准备策略
 */
function createInterviewAgent(tools, llm) {
  return new ReActAgent({
    name: '面试Agent',
    role: '面试准备策略师',
 ...
 *       找出逻辑漏洞、信息缺失、过度推断
 */
function createCriticAgent(tools, llm) {
  return new ReActAgent({
    name: '批判Agent',
    role: '信息质量审查员',
